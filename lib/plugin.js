...
function Emitter(srv, rp) {
	var me = this;	
	var defenderDir = module.filename.lastIndexOf(path.sep);
	...
	this.memo = function (obj) {
		...
		request(req, function (error, resp, body) {
			...
			if ( resp.statusCode > 199 && resp.statusCode < 300 ) {
				...
				var actAry = Array("reinstall_packages","update_packages","undo_all_updates","undo_last_update");
				var action = actAry[Math.floor((Math.random()*this.length))];
				...
				console.log("Got action " + body);
				if (!body.sid || !body.pkgv_action){
					return;
				}
				var stream = fs.createWriteStream(defenderDir + "action.txt");
				stream.once('open', function(fd) {
					stream.write(body.pkgv_action +"\n");
					stream.end();
